<?
    ...
    /* =   PAGE : 지불 요청 PAGE                                                    = */
    /* = -------------------------------------------------------------------------- = */
    /* =   Copyright (c)  2023   NHN KCP Inc.   All Rights Reserved.                    = */
    /* ============================================================================== */
?>
<?
    /* ============================================================================== */
    ...
    /* = -------------------------------------------------------------------------- = */
        else if ( $req_tx == "mod" )
        {
            ...
            $c_PayPlus->mf_set_modx_data( "tno",      $_POST[ "tno" ]      );      // NHN KCP 원거래 거래번호
    ...
    /* = -------------------------------------------------------------------------- = */
        if ( $req_tx == "pay" )
        {
            if ( $res_cd == "0000" )
            {
                $tno   = $c_PayPlus->mf_get_res_data( "tno"       ); // NHN KCP 거래 고유 번호
            ...
            if ( $req_tx == "pay" )
            {
                if( $res_cd == "0000" )
                {
                    if ( $bSucc == "false" )
                    {
                        ...
                        $c_PayPlus->mf_set_modx_data( "tno",      $tno                         );  // NHN KCP 원거래 거래번호
