	/* ++++++++++++++++++++++++++++++++++++
		웹 에디터 지원 스크립트
	*/
	var str_Icon_Path = "/pub/img/edit/";		// --- 아이콘 이미지 경로
	...
	arr_Edit_Icon[29] = new Array(str_Icon_Path + "box.gif", 'fncImgMode();', '이미지찾기');
	...
	function fncCngMode(fArea, vArea, vIframe, mode)
	{
		...
		if(mode>0)	// text => html
		{
			...
			vIframe.innerHTML='<iframe scrolling="yes" src="about:blank" name="I_Editor" id="html_Editor" style="width:100%;height:'+loc_Edit_Height+'px;"></iframe>';
	...
	function fnc_Out_Editor_Menu(pr_Form_Name, pr_Format_Input, pr_Mtx_Name)
	{
		...
		str_Sel_Format +=	'<table border="0" cellpadding="0" cellspacing="0" width="100%">'+
							'<tr>'+
								'<td>'+
									...
									'<input type="hidden" name="sel_Editor_Format"  class="sel_Css" value="9">';
		if(fnc_Brow_Info()>0)
		{
		...
		if(fnc_Brow_Info()>0){
			str_Sel_Format +=	'<tr id="viewEditMenu" style="display:none;">'+
								...
								'<tr>'+
									'<td>'+
										'<span id="viewArea" style="visibility:visible;"><textarea name="'+pr_Mtx_Name+'" class="mtx_Css" cols="65" rows="10" style="width:100%;height:'+loc_Edit_Height+'px;"></textarea></span>'+
