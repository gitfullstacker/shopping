...
<?
	...
	switch($RetrieveFlag){
     	case "INSERT" :
			...
			<script language="javascript">
				alert("회원가입을 축하드립니다.");
				window.location.href="join04.php";
			</script>
			<?
			exit;
			break;
     	...
     	case "IDCHECK" :
     		...
     		If ($str_userid!="") {
				...
				if($rcd_cnt){?>
					...
					<span class="f_bd f_bk"><?=$str_userid?></span>은 등록되어 있는 아이디입니다.
          			<input type="hidden" name="str_userid_chk" id="str_userid_chk" value="0" />

				<?}else{?>
					<span class="pl10"><span class="f_bd f_bk"><?=$str_userid?></span> 은 사용 가능합니다. </span>
