...
<div class="flex flex-col w-full px-[14px]">
    <div x-data="{
        ...
        <input type="text" class="w-full h-[38px] bg-[#F8F8F8] border border-solid border-[#E0E0E0] rounded-[4px] pl-3 pr-7 font-bold text-xs leading-[14px] placeholder:text-[#C4C4C4]" x-model="searchKey" placeholder="검색어를 입력해주세요.">
    ...
    <div class="mt-[25.17px] flex flex-col w-full gap-5">
        ...
        <p class="font-bold text-xs leading-[14px] text-[#9D9D9D]">최근 검색어가 없습니다.</p>
    </div>
        ...
        <div class="grid grid-cols-2 gap-0.5 w-full">
            ...
            ?>
                <button class="flex flex-col gap-0.5 px-3 py-[7px] bg-[#F8F8F8]" onclick="searchProduct(<?= $row['INT_NUMBER'] ?>)">
                    ...
                    <p class="font-bold text-[10px] leading-[11px] text-[#9D9D9D]"><?= $row['STR_KCODE'] ?></p>
                </button>
            <?php
            }
            ?>
