...
while ($row = mysql_fetch_assoc($event_list_result)) {
    $result .= '
        <a href="event_detail.php?int_number=' . $row['INT_NUMBER'] . '" class="flex flex-col w-full px-3 pb-[13.8px] border-b-[0.5px] border-[#E0E0E0]">
            ...
            <p class="mt-[3px] font-semibold text-xs leading-[14px] text-[#666666]">' . $row['STR_STITLE'] . '</p>
        </a>';
}
$result .= '</div>';
...
$result .= '
    <div class="mt-[30px] flex gap-[23px] justify-center items-center">
        <button type="button" onclick="searchEvent(' . (($page - 1) < 0 ? '0' : $page - 1) . ', ' . $int_type . ')">
...
for ($i = $start_page; $i <= $end_page; $i++) {
    $result .=
        '<button type="button" class="flex justify-center items-center w-[25.28px] h-[25.28px] border border-solid border-[#DDDDDD] ' . ($i == $page ? 'bg-black' : 'bg-white') . '" onclick="searchEvent(' . $i . ', ' . $int_type . ')">
            <p class="font-bold text-xs leading-[14px] text-center ' . ($i == $page ? 'text-white' : 'text-black') . '">' . $i . '</p>
        </button>';
}
$result .= '
        </div>
        <button type="button" onclick="searchEvent(' . (($page + 1) > $last_page ? $last_page : $page + 1) . ', ' . $int_type . ')">
            <svg width="8" height="17" viewBox="0 0 8 17" fill="none" xmlns="http://www.w3.org/2000/svg">
