...
while ($row = mysql_fetch_assoc($review_list_result)) {
    ...
    <div class="flex flex-col w-full py-5">
        ...
        <div class="mt-[9px] flex gap-5 items-center">
            ...
            <p class="font-bold text-xs leading-[14px] text-[#999999]">' . date('Y/m/d', strtotime($row['BD_REG_DATE'])) . '</p>
        </div>
        ...
        <div class="mt-[9px] flex flex-col gap-2 w-full">
            <img class="flex min-w-full" src="/admincenter/files/boad/2/' . $row['IMG_F_NAME'] . '" onerror="this.style.display=\'none\'" alt="" />
        </div>
    </div>';
}
$result .= '</div>';
