...
while ($row = mysql_fetch_assoc($review_list_result)) {
    ...
    <div class="flex flex-col w-full py-5">
        ...
        <div class="mt-[15px] flex justify-between items-center">
            <p class="font-bold text-xs leading-[14px] text-black">' . str_repeat('★', $row['BD_ITEM2']) . '</p>
            <button class="flex justify-center items-center gap-[2.7px] w-10 h-[17px] bg-white border-[0.7px] border-solid border-[#DDDDDD] rounded-full" onclick="setLike(' . $row['BD_SEQ'] . ')">
                ...
                <span id="like_count_' . $row['BD_SEQ'] . '" class="font-bold text-[9px] leading-[9px] text-[#666666]">' . $row['COUNT_LIKE'] . '</span>
            </button>
        </div>
        <div class="mt-[9px] flex gap-5 items-center">
            <p class="text-bold font-xs leading-[14px] text-[#666666]">' . substr($row['MEM_ID'], 0, 3) . '***</p>
            <p class="font-medium text-xs leading-[14px] text-[#999999]">' . date('Y/m/d', strtotime($row['BD_REG_DATE'])) . '</p>
        </div>
        <div x-data="{ isCollapsed: true }" class="mt-[9px] flex flex-col gap-2.5 w-full">
            <p class="font-medium text-xs leading-[18px] text-[#666666]" x-bind:class="isCollapsed ? \'line-clamp-4\' : \'\'">
        ...
        <div x-data="{ selectedImage: 0 }" class="mt-2.5 grid gap-2 w-full" x-bind:class="selectedImage == 0 ? \'grid-cols-3\' : \'grid-cols-1\'">
            ' . $images . '
        </div>
    </div>';
}
