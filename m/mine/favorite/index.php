...
$alarm_product_result = mysql_query($SQL_QUERY);

// 찜한 상품
$SQL_QUERY =    'SELECT 
                    B.*, C.STR_CODE
                FROM 
                    ' . $Tname . 'comm_member_like A
                LEFT JOIN
                    ' . $Tname . 'comm_goods_master B
                ON
                    A.STR_GOODCODE=B.STR_GOODCODE
...
<div class="mt-1.5 flex flex-col w-full min-h-screen px-[14px]">
    <div x-data="{
        ...
        <div class="flex justify-between">
            ...
            <button type="button" x-on:click="removeLikeAll()">
        ...
        <div x-data="{ noData: <?= mysql_num_rows($alarm_product_result) > 0 ? 'false' : 'true' ?> }" x-show="type == 1" class="mt-[18px] flex flex-col w-full" style="display: none;">
            ...
            <div x-show="!noData" id="alarm_list" class="grid grid-cols-2 gap-x-[13.5px] gap-y-[30px]">
                <?php
                while ($row = mysql_fetch_assoc($alarm_product_result)) {
                ?>
                    <!-- 상품 -->
                    <a href="/m/product/detail.php?str_goodcode=<?= $row['STR_GOODCODE'] ?>" class="flex flex-col w-full" id="alarm_product_item_<?= $row['STR_GOODCODE'] ?>">
                        <div class="relative flex justify-center items-center w-full h-[176px] p-2.5 bg-[#F9F9F9] rounded-[6px]">
                            ...
                            <div class="absolute top-[11px] right-[11px] flex justify-center items-center w-4 h-4" onclick="setAlarm(<?= $row['STR_GOODCODE'] ?>)">
        ...
        <!-- 찜목록 -->
        <div x-data="{ noData: <?= mysql_num_rows($favorite_product_result) > 0 ? 'false' : 'true' ?> }" x-show="type == 2" class="mt-[18px] flex flex-col w-full" style="display: none;">
            ...
            <div x-show="!noData" id="product_list" class="grid grid-cols-2 gap-x-[13.5px] gap-y-[30px]">
                <?php
                while ($row = mysql_fetch_assoc($favorite_product_result)) {
                ?>
                    <!-- 상품 -->
                    <a href="/m/product/detail.php?str_goodcode=<?= $row['STR_GOODCODE'] ?>" class="flex flex-col w-full" id="favorite_product_item_<?= $row['STR_GOODCODE'] ?>">
                        <div class="relative flex justify-center items-center w-full h-[176px] p-2.5 bg-[#F9F9F9] rounded-[6px]">
                            <img src="/admincenter/files/good/<?= $row['STR_IMAGE1'] ?>" alt="product">
...
<script>
            ...
            success: function(resultString) {
                ...
                if (result['status'] == 200) {
                    if (result['data'] == false) {
                        $("#favorite_product_item_" + str_goodcode).hide();
                    }
                }
            }
        });
    ...
    function setAlarm(str_goodcode) {
        $.ajax({
            url: "/m/product/set_alarm.php",
            data: {
                str_goodcode: str_goodcode
            ...
            success: function(resultString) {
                ...
                if (result['status'] == 200) {
                    if (result['data'] == false) {
                        $("#alarm_product_item_" + str_goodcode).hide();
                    }
                }
            }
        });
