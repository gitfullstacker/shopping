...
if ($end_page > 0) {
        ...
        $result .= '
            <div class="flex justify-between items-center pb-[15px] border-b-[0.5px] border-[#E0E0E0]">
                <div class="flex flex-col w-full">
                    ...
                    <p class="mt-1.5 font-bold text-xs leading-14px text-[#666666]">' . $mileage_type . '</p>
    ...
    $result .= '
        <div class="mt-[30px] flex gap-[23px] justify-center items-center">
            <button type="button" onclick="searchReserve(' . (($page - 1) < 0 ? '0' : $page - 1) . ')">
    ...
    for ($i = $start_page; $i <= $end_page; $i++) {
        $result .= '
            <button type="button" class="flex justify-center items-center w-[25.28px] h-[25.28px] border border-solid border-[#DDDDDD] ' . ($i == $page ? 'bg-black' : 'bg-white') . '" onclick="searchReserve(' . $i . ')">
                <p class="font-bold text-xs leading-[14px] text-center ' . ($i == $page ? 'text-white' : 'text-black') . '">' . $i . '</p>
            </button>';
    }
    $result .= '
            </div>
            <button type="button" onclick="searchReserve(' . (($page + 1) > $last_page ? $last_page : $page + 1) . ')">
                <svg width="8" height="17" viewBox="0 0 8 17" fill="none" xmlns="http://www.w3.org/2000/svg">
