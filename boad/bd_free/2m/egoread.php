...
<?
	...
	$Sql_Query .= " A.BD_SEQ=".$int_Bd_Seq." AND A.BD_ID_KEY IS NOT NULL AND A.MEM_ID='$arr_Auth[0]' ";
	...
	$str_Pri_Pwd = Fnc_Om_Conv_Default($_REQUEST[pwd],"");
		...
		If ($arr_Get_Data[0][12]>0) {
			...
			If ($bln_Flag==False) {
				...
				If ($str_Pri_Pwd=="") {
					...
					If ($str_Pri_Pwd!=$arr_Get_Data[0][21]) {
						echo "<Script Language='JavaScript'>alert('글암호가 일치하지 않습니다.');document.location.replace('egolist.php?bd=".$int_Conf_Seq."');</Script>";
						exit;
					}
				}
			}
...
<?include "inc/inc_mid.php";?>
						...
						<table class="t_type01">
							...
							<tbody>
									...
									<td class="left" colspan="3">
										<div class="prd_bx">
											<p class="img" style="width:120px;height:120px;">
												<?if ($arr_Get_Data[0][23]!=""){?><img src="/admincenter/files/good/<?=$arr_Get_Data[0][23]?>" style="width:120px;height:120px;" alt="" /><?}else{?>&nbsp;<?}?>
											</p>
											<div class="f_bd f_bk">
												<p><?=$arr_Get_Data[0][24]?></p>
												<p><?=$arr_Get_Data[0][22]?></p>
											</div>
										</div>
									</td>
								</tr>
								<tr>
									...
									<td class="left line_r"><?=$arr_Get_Data[0][4]?></td>
									<th class="left">성명</th>
									<td class="left">
										<?
											$str_Tmp = "";
								...
								<tr>
									<th class="left">제목</th>
									<td class="left f_bk f_bd" colspan="3"><?=stripslashes($arr_Get_Data[0][8])?></td>
								</tr>
								<tr>
									<th class="left">내용</th>
									<td class="left" colspan="3">
					...
					<div class="center mt30">
						<?If ($arr_Get_Data[0][4]=="" && $bln_Cur_Writer==False) {?>
							<input type="password" class="board_input" name="txt_Pwd" size="10" style="height:19px;">
						<?}?>
						<a href="#;" class="btn btn_l btn_bk w w270 f_bd" style="cursor:pointer;" onclick="fnc_Send_Data(document.frm_Send, 1);" align="absmiddle" alt="edit">수정하기</a>
						<a href="#;" class="btn btn_l btn_bk w w270 f_bd" style="cursor:pointer;" onclick="fnc_Send_Data(document.frm_Send, 0);" align="absmiddle" alt="delete">삭제</a>
						<a href="egolist.php<?=$str_String?>" class="btn btn_l btn_wt w w270 f_bd">목록</a>
					</div>
