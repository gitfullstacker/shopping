...
<input type="hidden" name="hid_Url">
	...
	<input type="hidden" name="mode" value="99">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="board_list" align="center">
							...
							?>
							  <tr>
								<td><span class="board_opt">공지</span></td>
								<td class="board_title">
									...
									<?
										...
										If (Trim(mysql_result($arr_Notice_Data,$int_I,mem_id))=="") {
											$int_Mem_Yn = 0;
										}

										$int_Match = 0;
										If ((Trim(mysql_result($arr_Notice_Data,$int_I,mem_id))==$arr_Auth[0])&&($arr_Auth[0]!="")) {
											$int_Match = 1;
										}
									?>
									<label onmouseover="this.style.cursor='hand';" onclick="fnc_Read_Cont('egoread.php<?=$str_String.$int_Cur_Page?>&seq=<?=mysql_result($arr_Notice_Data,$int_I,bd_seq)?>', <?=$int_Mem_Yn?>, <?=mysql_result($arr_Notice_Data,$int_I,bd_open_yn)?>, <?=$int_Adm?>, <?=$int_Match?>);">
									<?=$str_Tmp?>
									<?
										// ========================
										//	= 메모글 갯수 출력 시작
										...
										if (strlen(stripslashes(mysql_result($arr_Notice_Data,$int_I,bd_title))) > 30 ) {
											$str_Tmp = mb_substr(stripslashes(mysql_result($arr_Notice_Data,$int_I,bd_title)),0,30,'utf-8')."...";
										} else {
								...
								<td>
									<?
										If (mysql_result($arr_Notice_Data,$int_I,img_seq)+mysql_result($arr_Notice_Data,$int_I,file_seq)>0) {
											echo "<img src='/images/board/icon_disk.jpg' align='absMiddle'>";
										}Else{
											echo "&nbsp;";
										}
									?>
								...
								<td><?=substr(mysql_result($arr_Notice_Data,$int_I,bd_reg_date),0,10)?></td>
								<td><?=mysql_result($arr_Notice_Data,$int_I,bd_view_cnt)?></td>
							  </tr>
							<?}?>
							<?if($int_Total_Cnt!=0){?>
							  ...
							  <tr>
								...
								<td class="board_title">
									...
									<?
										...
										If (Trim(mysql_result($arr_Get_Data,$int_I,mem_id))=="") {
											$int_Mem_Yn = 0;
										}

										$int_Match = 0;
										If ((Trim(mysql_result($arr_Get_Data,$int_I,mem_id))==$arr_Auth[0])&&($arr_Auth[0]!="")) {
											$int_Match = 1;
										}
									?>
									<label onmouseover="this.style.cursor='hand';" onclick="fnc_Read_Cont('egoread.php<?=$str_String.$int_Cur_Page?>&seq=<?=mysql_result($arr_Get_Data,$int_I,bd_seq)?>', <?=$int_Mem_Yn?>, <?=mysql_result($arr_Get_Data,$int_I,bd_open_yn)?>, <?=$int_Adm?>, <?=$int_Match?>);">
									<?=$str_Tmp?>
									<?
										// &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
										//	= 새글표시 시작
									...
									<?
										...
										if (strlen(stripslashes(mysql_result($arr_Get_Data,$int_I,bd_title))) > 40 ) {
											$str_Tmp = mb_substr(stripslashes(mysql_result($arr_Get_Data,$int_I,bd_title)),0,40,'utf-8')."...";
										} else {
											$str_Tmp = stripslashes(mysql_result($arr_Get_Data,$int_I,bd_title));
										}
								...
								<td><?=substr(mysql_result($arr_Get_Data,$int_I,bd_reg_date),0,10)?></td>
								<td><?=mysql_result($arr_Get_Data,$int_I,bd_view_cnt)?></td>
							  </tr>
							<?
							$article_num--;
							if($article_num==0){
							...
							<?}else{?>
							<tr>
								<td colspan="6" align="center" valign="middle" height="200">자료가 없습니다.</td>
							</tr>
							<?}?>
							</tbody>
						</table>
						...
						<div class="board_search">
							...
							<legend style="display:none;">게시판 검색</legend>
								<select name="itm" size="1" style="width:90px;height:19px;" />
								...
								<input type="text" name="txt" maxlength="20" value="<?=$str_Txt?>" title="검색어를 입력하세요." class="board_input" style="width:200px;" />
								<input type="image" src="/images/board/btn_search.jpg" alt="" align="absmiddle" class="btn_mb1" />
							</fieldset>
						</div>
                    ...
                    <div class="board_button">
						...
						?>
						<a href="egowrite.php<?=$str_String . $str_Pg?>"><img src="/images/board/btn_write.jpg" alt="작성" /></a>
						<?
							}
