...
<input type="hidden" name="hid_Url">
	...
	<input type="hidden" name="mode" value="99">
			...
			<p class="f_size_s pt30">총 <?=$int_Total_Cnt?>개의 이용후기가 있습니다.</p>
			<div class="pt10">
				<a href="/boad/bd_free/m2m/egolist.php?bd=2" class="btn btn_ml btn_wt w100p">이용후기 작성하기</a>
			</div>
			<div class="t_cover01 mt10">
				<ul class="item_listyong">
					<?if($int_Total_Cnt!=0){?>
					<?$article_num = $int_Total_Cnt - $int_Page_Size*($str_Pg-1) ;?>
					<?for($int_I = 0 ;$int_I <= $int_Page_Size -1; $int_I++) {?>
					<li>
						<a href="javascript:menu('<?=mysql_result($arr_Get_Data,$int_I,bd_seq)?>');">
							<p class="item_img"><?if (mysql_result($arr_Get_Data,$int_I,str_image1)!=""){?><img src="/admincenter/files/good/<?=mysql_result($arr_Get_Data,$int_I,str_image1)?>" class="img_bd" alt="" /><?}else{?>&nbsp;<?}?></p>
							<div class="item_info">
								...
								<p class="date"><?=str_replace("-", ".",substr(mysql_result($arr_Get_Data,$int_I,bd_reg_date),0,10))?> <i class="icn icn_bar"></i> <?for ($int_C=0;$int_C<=strlen(mysql_result($arr_Get_Data,$int_I,mem_id))-4;$int_C++) {?><?=substr(mysql_result($arr_Get_Data,$int_I,mem_id),$int_C,1)?><?}?>***</p>
								<p class="icn_photo">
									<?
					...
					<li class="review_con" id="submenu_prodeval<?=mysql_result($arr_Get_Data,$int_I,bd_seq)?>" style="display: none;">
						<p class="item_img02">
							<?
							$Sql_Query =	" SELECT
											1 AS TYPE,
			...
			<!--<p class="mt15"><a href="#;" class="btn btn_readmore">READ MORE <i class="icn"></i></a></p>//-->
