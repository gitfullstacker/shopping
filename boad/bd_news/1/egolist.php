...
<?include "inc/inc_mid.php";?>
	...
	<input type="hidden" name="mode" value="99">
					...
					<div class="search_bx mt30">
						<span class="tit">공지사항 검색</span>
						<select name="itm" size="1" class="SlectBox">
							<option value="0"<?If ($int_Itm=="0") {?> selected<?}?>>글제목</option>
							<option value="1"<?If ($int_Itm=="1") {?> selected<?}?>>작성자</option>
						</select>
						<input type="text" name="txt" maxlength="20" value="<?=$str_Txt?>" class="inp" />
						<input type="submit" value="검색" class="btn_bt" />
					</div>
					<p class="f_s13 mt35"> 게시물 <?=$int_Total_Cnt?>건 / 총 페이지 : <b><?=$int_Total_Page?></b> / <?=$str_Pg?></p>
					<div class="t_cover01 mt15">
						<table class="t_type01">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<?
											If (mysql_result($arr_Notice_Data,$int_I,img_seq)+mysql_result($arr_Notice_Data,$int_I,file_seq)>0) {
												echo "<img src='/images/board/icn_file.gif' align='absMiddle'>";
											}Else{
												echo "&nbsp;";
											}
										?>
									...
									<td class="left">
										<?
											...
											// &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
										?>
										<a href="egoread.php<?=$str_String.$str_Pg?>&seq=<?=mysql_result($arr_Notice_Data,$int_I,bd_seq)?>" class="f_bk">
										<?=$str_Tmp?>
										<?
											// ========================
											//	= 메모글 갯수 출력 시작
								...
								<tr>
									...
									<td class="left">
										...
										<?
											...
											// &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
										?>
										<a href="egoread.php<?=$str_String.$str_Pg?>&seq=<?=mysql_result($arr_Get_Data,$int_I,bd_seq)?>" class="f_bk">
										<?=$str_Tmp?>
										<?
											// ========================
											//	= 메모글 갯수 출력 시작
								...
								<?}else{?>
								<tr>
									<td colspan="4" valign="middle" style="height:200px;" align="center">자료가 없습니다.</td>
								</tr>				
								<?}?>
							</tbody>
						</table>
