...
$SQL_QUERY .= $Tname;
$SQL_QUERY .= "comm_cal a where a.int_number is not null ";
$SQL_QUERY .= $Str_Query;
...
<head>
    ...
    <script language="javascript" src="js/calendar_list.js"></script>
</head>
                ...
                <table width=100%>
                    <tr>
                        <td style="padding:10px">
                            ...
                            <form id="frm" name="frm" target="_self" method="POST" action="calendar_list.php">
                                ...
                                <table class=tb>
                                    ...
                                    <tr>
                                        <td>제목</td>
                                        <td colspan="3">
                                            <input type="text" NAME="Txt_title" value="<?= $Txt_title ?>" style="width:300px;" onkeydown="javascript: if (event.keyCode == 13) {fnc_search();}">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>등록일</td>
                                    ...
                                    <? if ($total_record_limit != 0) { ?>
                                        ...
                                        <? for ($i = 0; $i <= $displayrow - 1; $i++) { ?>
                                            <tr height=30 align="center">
                                                ...
                                                <td style="text-align:center;"><?= mysql_result($result, $i, str_title) ?></td>
                                                <td style="text-align:center;">
                                                    <?php
                                                    $weeks = array('일', '월', '화', '수', '목', '금', '토');
                                                    $period = mysql_result($result, $i, int_period) == 0 ? '한번 ' : '매주/매월 ';
                                                    $date = '';
                                                    switch (mysql_result($result, $i, int_type)) {
                                                        case 1:
                                                            $date = mysql_result($result, $i, str_day) . '일';
                                                            break;
                                                        case 2:
                                                            $date = mysql_result($result, $i, str_date);
                                                            break;
                                                        case 3:
                                                            $date = $weeks[(int)mysql_result($result, $i, str_week)] . '요일';
                                                            break;
                                                    }
                                                    echo ($period . $date);
                                                    ?>
                                                </td>
                                                <td style="text-align:center;"><?= mysql_result($result, $i, int_dtype) == 1 ? '시작불가' : '반납불가' ?></td>
                                                <td>
