...
function fnc_search() {
	...
	document.frm.action = "event_list.php";
	document.frm.submit();
}
function Adelete_Click() {
	...
	if (frm.txtRows.value == 0) {
		...
		if (!count) {
			...
			document.frm.action = "event_edit_proc.php";
...
function fnc_re_f(Obj, int_number) {
	if (((Obj.checked) ? "Y" : "N") == "Y") {
		document.getElementById("idView_Re" + int_number).style.display = "none";
	} else {
		document.getElementById("idView_Re" + int_number).style.display = "";
	}
	fuc_set('event_edit_proc.php?RetrieveFlag=REF&str_no=' + int_number + '&str_auto_good=' + ((Obj.checked) ? "Y" : "N"), '_Proc');
}