...
$str_Add_Tag = $_SERVER['DOCUMENT_ROOT'] . "/admincenter/files/plan/";
...
?>
		...
		$SQL_QUERY = 	"INSERT INTO " . $Tname . "comm_plan 
		...
		?>
		<script language="javascript">
			window.location.href = "plan_edit.php?RetrieveFlag=UPDATE&str_no=<?= $lastnumber ?>";
		</script>
		<?
		exit;
		break;
	...
	case "UPDATE":
		...
		$SQL_QUERY = " UPDATE " . $Tname . "comm_plan SET ";
		...
		?>
		<script language="javascript">
			window.location.href = "plan_edit.php?RetrieveFlag=UPDATE&str_no=<?= $str_no ?>";
		</script>
	<?
		exit;
		break;
	...
	case "ADELETE":
		...
		for ($i = 0; $i < count($chkItem1); $i++) {
			...
			$SQL_QUERY =	"DELETE FROM " . $Tname . "comm_plan WHERE INT_NUMBER='$chkItem1[$i]' ";
			mysql_query($SQL_QUERY);
		}
	?>
		<script language="javascript">
