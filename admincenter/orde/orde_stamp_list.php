...
<?
	...
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_stamp_order a left join ".$Tname."comm_member b on a.str_userid=b.str_userid left join ".$Tname."comm_stamp_prod c on a.int_prod=c.int_prod where a.int_number is not null ";
	$SQL_QUERY.=$Str_Query;
	...
	$SQL_QUERY = "select a.*,b.str_name,c.str_prod from ";
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_stamp_order a left join ".$Tname."comm_member b on a.str_userid=b.str_userid left join ".$Tname."comm_stamp_prod c on a.int_prod=c.int_prod ";
...
<script language="javascript" src="js/orde_stamp_list.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="orde_stamp_list.php">
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<tr height=30 align="center">
								...
								<td><?=mysql_result($result,$i,str_prod)?></td>
								<td>
									<select name="str_pass" onchange="Click_Pass('<?=mysql_result($result,$i,int_number)?>',this.value)">
										<option value="0" <?if (mysql_result($result,$i,str_pass)=="0"){?> selected<?}?>>접수
										<option value="1" <?if (mysql_result($result,$i,str_pass)=="1"){?> selected<?}?>>완료
									</select>
								</td>
