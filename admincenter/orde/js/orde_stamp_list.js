	function fnc_search() {
		...
		document.frm.action = "orde_stamp_list.php";
		document.frm.submit();
	}
		...
		if (frm.txtRows.value==0) {
			...
			if (rowCnt >1){
	  		...
	  		if (!count) {
				...
				document.frm.action = "orde_stamp_edit_proc.php";
	...
	function Click_Pass(str_no,str_pass) {
		fuc_ajax('orde_stamp_edit_proc.php?RetrieveFlag=PASS&str_no='+str_no+'&str_pass='+str_pass);
		alert("처리되었습니다.");
	}