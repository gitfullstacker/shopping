...
<?
	...
	switch($RetrieveFlag){
     	...
     	case "UPDATE" :
			...
			$str_Temp=Fnc_Om_File_Save($str_Image8,$str_Image8_name,$str_dimage8,'','',$str_del_img6,$str_Add_Tag);
			if ($str_Temp[0] == "0") {
				?>
				<script language="javascript">
					alert("업로드에 실패하셨습니다.");
			...
			$str_Temp=Fnc_Om_File_Save($str_Image9,$str_Image9_name,$str_dimage9,'','',$str_del_img6,$str_Add_Tag);
			if ($str_Temp[0] == "0") {
				?>
				<script language="javascript">
					alert("업로드에 실패하셨습니다.");
			...
			$arr_Column_Name[22]		= "STR_TAG";
			$arr_Column_Name[23]		= "INT_LIKE";
			$arr_Column_Name[24]		= "STR_SERVICE";
			$arr_Column_Name[25]		= "STR_MYN";
			...
			$arr_Set_Data[22]		= $str_tag;
			$arr_Set_Data[23]		= $int_like;
			$arr_Set_Data[24]		= $str_service;
			$arr_Set_Data[25]		= $str_myn;
			...
			$Sql_Query = "DELETE FROM `".$Tname."comm_goods_master_sub` WHERE STR_GOODCODE='".$str_no."' ";
			mysql_query($Sql_Query);
			...
			for($ii = 0; $ii < $int_bsu; $ii++) {
				...
				$arr_Set_Data2[0]		= $str_no.Fnc_Om_Add_Zero($ii+1,3);
