...
function fnc_search() {
	...
	document.frm.action = "good_master_list.php";
	document.frm.submit();
}
...
function fnc_sort(str_goodcode, Txt_bcode, int_sort, str_way) {
	fnc_Iframe()
	lbl_Iframe.window.location.href = "good_master_list_proc.php?str_goodcode=" + str_goodcode + "&Txt_bcode=" + Txt_bcode + "&int_sort=" + int_sort + "&RetrieveFlag=" + str_way;
	//window.location.href="good_master_list_proc.php?str_goodcode="+str_goodcode+"&Txt_bcode="+Txt_bcode+"&int_sort="+int_sort+"&RetrieveFlag="+str_way;
}
...
function fnc_myn(str_no, str_myn) {
	fuc_ajax('good_master_edit_proc.php?RetrieveFlag=MYN&str_no=' + str_no + '&str_myn=' + str_myn);
	alert("수정되었습니다.");
}

function fnc_mmyn(str_no, str_mmyn) {
	fuc_ajax('good_master_edit_proc.php?RetrieveFlag=MMYN&str_no=' + str_no + '&str_mmyn=' + str_mmyn);
	alert("수정되었습니다.");
}

function Aaction_Click(type = 1) {
	if (frm.txtRows.value == 0) {
		alert("데이터가 존재하지 않습니다.");
		return;
	} else {
		...
		if (!count) {
			...
			if (document.frm.to_int_type.value === "") {
				alert("이동하려는 곳을 지정해주세요.");
				return;
			}
			if (!confirm("정말로 " + (type == 1 ? "이동" : "복사") + "하시겠습니까?")) return
			document.frm.encoding = "multipart/form-data";
			document.frm.action = "good_master_edit_proc.php";
			if (type == 1) {
				document.frm.RetrieveFlag.value = "AMOVE";
			} else {
				document.frm.RetrieveFlag.value = "ACOPY";
			}
			document.frm.submit();
		}
	}
