...
<?
	...
	If ($Txt_word!="") {
		switch ($Txt_key) {
			case  "all" :
				$Str_Query = " and (a.str_prod like '%$Txt_word%') ";
				break;
			case  "str_prod" :
				$Str_Query = " and a.str_prod like '%$Txt_word%' ";
				break;
	...
	$SQL_QUERY="select count(a.int_prod) from ";
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_stamp_prod a where a.int_prod is not null ";
	$SQL_QUERY.=$Str_Query;
...
<script language="javascript" src="js/good_stamp_list.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="good_stamp_list.php">
							...
							<tr>
								...
								<td colspan="3">
									<select name="Txt_key">
										...
										<option value="str_prod" <?If ($Txt_key=="str_prod") {?>selected<?}?>> 상품명 </option>
									</select>
