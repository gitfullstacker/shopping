											...
											<td bgcolor=#f6f6f6 style="padding:9px 0 9px 28px; border-bottom:1px solid #cacaca;">
												<table>
													<? while ($srow = mysql_fetch_array($arr_Rlt_Sub)) { ?>
														<tr>
															<td style="font:11px/22px dotum;letter-spacing:-1px;padding-left:10px;background:url('/admincenter/img/blet_admin_menu.gif') no-repeat 0 8px;"><a href="javascript:fnc_menu('<?= $srow[Str_CHOCODE] . $srow[Str_UNICODE] ?>','')" name=navi><? if ($arr_Auth[7] == $srow[Str_CHOCODE] . $srow[Str_UNICODE]) { ?><b style="color:#248edf;"><?= $srow[Str_IDXWORD] ?></b><? } else { ?><?= $srow[Str_IDXWORD] ?><? } ?></a></td>
														</tr>
													<? } ?>
												</table>
											</td>
