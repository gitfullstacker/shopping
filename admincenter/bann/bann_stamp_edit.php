...
<?
	...
	$int_gubun = Fnc_Om_Conv_Default($_REQUEST[int_gubun],"7");
...
<script language="javascript" src="js/bann_stamp_edit.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="bann_stamp_edit.php" enctype="multipart/form-data">
						...
						<table class=tb>
							...
							<tr>
								...
								<td colspan=3>
									...
									<table class=tb>
										...
										<tr>
											<td width="100%" align="center" valign="middle" height="400"><?if ($RetrieveFlag=="UPDATE") {?><?if (!($arr_Data['STR_IMAGE1']=="")) {?><img src="/admincenter/files/bann/<?=$arr_Data['STR_IMAGE1']?>" width="800" border="1"><?}else{?>등록된 사진이<br>없습니다.<?}?><?}else{?>이미지 등록이<br>필요합니다.<?}?></td>
										</tr>
									</table>
								</td>
							</tr>
							<tr>
								<td>이미지선택</td>
								<td colspan=3><input type=file name=str_image1 style="width:300px;">  (PC : 가로 : 1100 / 모바일 : 가로 자유)  <?if (!($arr_Data['STR_IMAGE1']=="")) {?> 삭제시 <input type="checkbox" name="str_del_img1" value="Y" class="null"><?}?></td>
							</tr>
