...
<script language="javascript" src="js/bann_notice_edit.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="bann_notice_edit.php" enctype="multipart/form-data">
						...
						<table class=tb>
							...
							<tr>
								<td>제목</td>
								<td colspan=3><input type=text name=str_title style="width:400px;" value="<?=$arr_Data['STR_TITLE']?>"></td>
							</tr>
							<tr>
								<td>내용</td>
								<td colspan=3><textarea name=str_cont style="width:100%;height:50px;"><?=$arr_Data['STR_CONT']?></textarea></td>
							</tr>
							<tr>
								<td>이미지</td>
								<td colspan=3>
									<table class=tb>
										...
										<tr>
											<td width="100%" align="center" valign="middle" height="300"><?if ($RetrieveFlag=="UPDATE") {?><?if (!($arr_Data['STR_IMAGE1']=="")) {?><img src="/admincenter/files/bann/<?=$arr_Data['STR_IMAGE1']?>" width="336" height="243" border="1"><?}else{?>등록된 사진이<br>없습니다.<?}?><?}else{?>이미지 등록이<br>필요합니다.<?}?></td>
										</tr>
									</table>
								</td>
							</tr>
							<tr>
								<td>이미지선택</td>
								<td colspan=3><input type=file name=str_image1 style="width:300px;">  (336*243)  <?if (!($arr_Data['STR_IMAGE1']=="")) {?> 삭제시 <input type="checkbox" name="str_del_img1" value="Y" class="null"><?}?></td>
							</tr>
							<tr>
								<td>링크</td>
						...
						<a href='bann_notice_list.php'><img src='/admincenter/img/btn_list.gif'></a>
						</div>
