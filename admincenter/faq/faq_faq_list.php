...
<?
	...
	$displayrow = Fnc_Om_Conv_Default($_REQUEST[displayrow],60);
	$displaypage = Fnc_Om_Conv_Default($_REQUEST[displaypage],10);

	$Txt_quest  = Fnc_Om_Conv_Default($_REQUEST[Txt_quest],"");
	...
	If ($Txt_quest!="") { $Str_Query .= " and a.str_quest like '%$Txt_quest%' ";}
	If ($Txt_service!="") { $Str_Query .= " and a.str_service like '%$Txt_service%' ";}
	...
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_faq a where a.int_number is not null ";
	$SQL_QUERY.=$Str_Query;
...
<script language="javascript" src="js/faq_faq_list.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="faq_faq_list.php">
						...
						<table class=tb>
							...
							<tr>
								<td>키워드검색</td>
								<td>
									<input type="text" NAME="Txt_quest" value="<?=$Txt_quest?>" style="width:300px;" onkeydown="javascript: if (event.keyCode == 13) {fnc_search();}">
								</td>
								<td>출력여부/분류</td>
								<td>
									<select name="Txt_service">
										<option value="" selected> 전체 </option>
										<option value="Y" <?If ($Txt_service=="Y") {?>selected<?}?>> 출력 </option>
										<option value="N" <?If ($Txt_service=="N") {?>selected<?}?>> 미출력 </option>
									</select>
									/
									<?
									...
									<select name=Txt_gubun style="width:200px;">
										...
										<option value="<?=$row[INT_NUMBER]?>" <?if ($row[INT_NUMBER]==$Txt_gubun) {?>selected<?}?>><?=$row[STR_CODE]?>
										<?}?>
									</select>
								</td>
							</tr>
							<tr>
								<td>등록일자</td>
								<td colspan="3">
									<input type=text name=Txt_sindate value="<?=$Txt_sindate?>" id="Txt_sindate" onclick="displayCalendar(document.frm.Txt_sindate ,'yyyy-mm-dd',this)" readonly>
									...
									<img src="/pub/img/icons/calendar_delete.gif" align="absmiddle" style="cursor:hand" onclick="document.frm.Txt_sindate.value=''";>
									 -
									<input type=text name=Txt_eindate value="<?=$Txt_eindate?>" id="Txt_eindate" onclick="displayCalendar(document.frm.Txt_eindate ,'yyyy-mm-dd',this)" readonly>
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<tr height=30 align="center">
								...
								<td><font color=616161><?=mysql_result($result,$i,str_code)?></font></td>
								<td style="text-align:left;"><?=mysql_result($result,$i,str_quest)?></td>
								...
								<td><font class=small color=616161><?If (mysql_result($result,$i,str_mservice)=="Y"){?>출력<?}else{?>미출력<?}?></font></td>
