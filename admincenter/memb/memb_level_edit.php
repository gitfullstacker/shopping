...
<script language="javascript" src="js/memb_level_edit.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="memb_level_edit.php">
						...
						<table class=tb>
							...
							<tr>
								...
								<td nowrap>
								상품 판매 금액의
								<select name="int_persent" style="width:150;">
									<?for ($i = 0; $i<=100; $i++) {?>
									<option value="<?=$i?>" <?If (Trim($i)==Trim($arr_Data['INT_PERSENT'])) {?>Selected<?}?>><?=$i?>%
									<?}?>
								</select>
								할인 혜택을 제공 합니다. (할인혜택 안주려면 0%으로 처리)
								</td>
							</tr>
							<tr>
								<td nowrap>사용유무</td>
						...
						<a href='memb_level_list.php?str_idxcode=<?=$str_idxcode?>'><img src='/admincenter/img/btn_list.gif'></a>
						</div>
