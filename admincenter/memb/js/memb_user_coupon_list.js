function RowClick(str_no) {
	...
	document.frm.action = "memb_user_coupon_edit.php"
	document.frm.submit();
}
function AddNew() {
...
function fnc_search() {
	...
	document.frm.action = "memb_user_coupon_list.php";
	document.frm.submit();
}
function Adelete_Click() {
	...
	if (frm.txtRows.value == 0) {
		...
		if (!count) {
			...
			document.frm.action = "memb_user_coupon_edit_proc.php";
