	function RowClick(str_no) {
		...
		document.frm.action = "memb_level_edit.php"
		document.frm.submit();
	}
	function AddNew() {
	...
	function fnc_search() {
		...
		document.frm.action = "memb_level_list.php";
		document.frm.submit();
	}
	function Adelete_Click() {
		...
		if (frm.txtRows.value==0) {
			...
			if (rowCnt >1){
	  			...
	  			if (document.getElementsByName('chkItem1[]').checked) {
	  				count++;
	  			}		  		
			}
	  		...
	  		if (!count) {
				...
				document.frm.action = "memb_level_edit_proc.php";
