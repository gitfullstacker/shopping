...
<script language="javascript" src="js/memb_level_list.js"></script>
</head>
<body class=scroll> 
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="memb_level_list.php">
						...
						<table width=100%>
							<tr>
								...
								<?switch($str_idxcode){
     								case "06" :?>
								<select name="str_level" style="width:150px;"onChange="fuc_set('/admincenter/memb/memb_level_edit_proc.php?RetrieveFlag=LEVEL1&str_level='+this.value,'_Proc');">
									<option value="00">선택
									<?while($row=mysql_fetch_array($arr_code_Data)){?>
									<option value="<?=$row[STR_IDXNUM]?>" <?if (trim($row[STR_IDXNUM])==trim(Fnc_Om_Store_Info(9))) {?>selected<?}?>><?=$row[STR_IDXWORD]?>
									<?}?>
								</select>
								<?break;?>
								<?case "07" :?>
								<select name="str_level" style="width:150px;"onChange="fuc_set('/admincenter/memb/memb_level_edit_proc.php?RetrieveFlag=LEVEL2&str_level='+this.value,'_Proc');">
									<option value="00">선택
									<?while($row=mysql_fetch_array($arr_code_Data)){?>
									<option value="<?=$row[STR_IDXNUM]?>" <?if (trim($row[STR_IDXNUM])==trim(Fnc_Om_Store_Info(10))) {?>selected<?}?>><?=$row[STR_IDXWORD]?>
									<?}?>
								</select>								
								<?break;?>
								<?}?>
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<tr height=30 align="center">
								...
								<td><span id="navig" name="navig" m_id="admin" m_no="1"><font color=0074BA><b><?=mysql_result($result,$i,str_idxword)?></b></font></span></td>
								<td><?=mysql_result($result,$i,int_persent)?>%</td>
