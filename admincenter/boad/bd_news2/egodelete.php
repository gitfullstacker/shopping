			...
			If ($arr_Temp_Cnt>0) {
				...
				for($int_Z = 0 ;$int_Z < $arr_Data_Cnt; $int_Z++) {
				   	if (mysql_result($arr_Data,$int_Z,str_image1) !="") {
				   		Fnc_Om_File_Delete($str_Add_Tag, mysql_result($arr_Data,$int_Z,str_image1));
				   	}
				}


				$Sql_Query =	"DELETE FROM ".$Tname."comm_recruit WHERE INT_PNUMBER=".$arr_Bd_Seq[$int_I];
				$result=mysql_query($Sql_Query);
