...
<?
	...
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_requ a where a.int_number is not null and a.str_pass in ('0','1') ";
	$SQL_QUERY.=$Str_Query;
	...
	$SQL_QUERY.="where a.int_number is not null and a.str_pass in ('0','1') ";
	$SQL_QUERY.=$Str_Query;
...
<script language="javascript" src="js/requ_requ_list.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="requ_requ_list.php">
							...
							<tr>
								...
								<td>
									<select name="Txt_pass">
										<option value="" selected> 선택 </option>
										<option value="0" <?If ($Txt_pass=="0") {?>selected<?}?>> 접수 </option>
										<option value="1" <?If ($Txt_pass=="1") {?>selected<?}?>> 상담완료 </option>
									</select>
								</td>
							</tr>
							<tr>
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<tr height=30 align="center">
								...
								<td><?=number_format(mysql_result($result,$i,str_price))?></td>
								<td><?=mysql_result($result,$i,int_gubun)?></td>
								<td><font color=616161>
									<?switch (mysql_result($result,$i,str_pass)) {
										case  "0" : echo "접수"; break;
										case  "1" : echo "상담완료"; break;
									}
									?>
								</font></td>
