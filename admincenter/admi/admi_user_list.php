...
<?
	...
	$Txt_menu_level = Fnc_Om_Conv_Default($_REQUEST[Txt_menu_level],"00");
	...
	If ($Txt_word!="") {
		switch ($Txt_key) {
			case  "all" :
				$Str_Query = " and (a.str_name like '%$Txt_word%' or a.str_userid like '%$Txt_word%' or a.str_email like '%$Txt_word%' or a.str_telep like '%$Txt_word%') ";
				break;
			case  "str_name" :
	...
	If ($Txt_menu_level!="00") { $Str_Query .= " and a.str_menu_level = '$Txt_menu_level' ";}
	...
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_member a where a.int_gubun=91 ";
	$SQL_QUERY.=$Str_Query;
	...
	$SQL_QUERY = "select a.*,b.str_idxword from ";
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_member a ";
	$SQL_QUERY.="left join  ";
	$SQL_QUERY.=$Tname;
	$SQL_QUERY.="comm_fun_code b on b.str_idxcode='01' and a.str_menu_level=b.str_idxnum where a.int_gubun=91 ";
	$SQL_QUERY.=$Str_Query;
...
<script language="javascript" src="js/admi_user_list.js"></script>
</head>
<body class=scroll>
			...
			<table width=100%>
				<tr>
					<td style="padding:10px">
						...
						<form id="frm" name="frm" target="_self" method="POST" action="admi_user_list.php">
						...
						<table class=tb>
							...
							<tr>
								...
								<td>승인여부<font class=small color=444444>/</font>그룹</td>
								<td>
									...
									?>
									<select name="Txt_menu_level">
										<option value="00">↓그룹선택</option>
										<?while($row=mysql_fetch_array($arr_code_Data)){?>
										<option value="<?=$row[STR_IDXNUM]?>" <?if (trim($Txt_menu_level)==trim($row[STR_IDXNUM])) {?>selected<?}?>><?=$row[STR_IDXWORD]?>
										<?}?>
									</select>
								</td>
							</tr>
						...
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							...
							<tr height=30 align="center">
								...
								<td><?=mysql_result($result,$i,str_idxword)?></td>
						...
						<img src="/admincenter/img/btn_regist_s.gif" alt="등록" border=0 align=absmiddle style="cursor:pointer" onClick="AddNew();">
						</div>
						</form>
